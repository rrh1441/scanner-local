...
export class StandardErrorHandler {
    ...
    logError(error, context, level = 'error') {
        const log = (...args) => rootLog(`[${context.moduleName}]`, ...args);
        const errorData = {
     ...
     */
    async createErrorArtifact(error, context, severity = 'MEDIUM', scanDurationMs) {
        if (!context.scanId)
            return;
    ...
    isRetryableError(error, retryConfig) {
        ...
        return retryConfig.retryableErrors?.some(retryableError => errorMessage.includes(retryableError.toLowerCase()) ||
            errorName.includes(retryableError.toLowerCase())) ?? false;
    }
    /**
     ...
     */
    async withRetry(operation, context, retryConfig = {}) {
     ...
     */
    async executeModule(moduleName, operation, context = {}) {
        const startTime = Date.now();
        const fullContext = { ...context, moduleName };
        const log = (...args) => rootLog(`[${moduleName}]`, ...args);
        try {
    ...
    isCriticalSystemError(error) {
        ...
        return criticalErrors.some(criticalError => error.message.includes(criticalError) || error.name.includes(criticalError));
    }
}
 ...
 */
export async function executeModule(moduleName, operation, context) {
 ...
 */
export async function apiCall(operation, context) {
 ...
 */
export async function fileOperation(operation, context) {
    return errorHandler.withRetry(operation, context, {
        maxAttempts: 2,
        baseDelayMs: 500,
