 ...
 */
export async function uploadFile(localPath, key, mimeType) {
    try {
        ...
        log(`[objectStore] Failed to upload file ${localPath}:`, error.message);
...
export async function getDownloadUrl(key, expiresIn = 3600) {
    try {
        ...
        log(`[objectStore] Failed to generate download URL for ${key}:`, error.message);
        throw error;
    }
}
/**
