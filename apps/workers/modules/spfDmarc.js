 ...
 */
async function resolveSpfRecord(domain, lookups = 0, redirectChain = []) {
    ...
    try {
        ...
        if (records.length > 1)
            return { record: records.join(' | '), lookups, error: 'MULTIPLE_RECORDS', allMechanism: 'none' };
        const record = records[0];
        const mechanisms = record.split(' ').slice(1);
        let currentLookups = lookups;
        let finalResult = { record, lookups, allMechanism: 'none' };
        for (const mech of mechanisms) {
            if (mech.startsWith('include:')) {
                ...
                if (result.error)
                    return { ...finalResult, error: result.error, lookups: currentLookups };
            }
...
export async function runSpfDmarc(job) {
    ...
    if (spfResult.error === 'NONE_FOUND') {
        ...
        await insertFinding({
            ...
            severity: 'HIGH',
            title: 'Invalid SPF record detected',
        ...
        if (spfResult.allMechanism === '~all' || spfResult.allMechanism === '?all') {
            ...
            await insertFinding({
                ...
                type: 'EMAIL_SECURITY_WEAKNESS',
                severity: 'MEDIUM',
                title: 'Weak SPF policy detected',
    ...
    const commonSelectors = [
        ...
        'mailchimp', 'constantcontact', 'hubspot', 'salesforce',
    ...
    if (!dkimFound) {
        ...
        await insertFinding({
            ...
            type: 'EMAIL_SECURITY_GAP',
            severity: 'HIGH',
            title: 'No valid DKIM record found',
