...
async function validateSSLScan() {
    try {
        ...
        log(`[tlsScan] [CRITICAL] sslscan binary not found: ${error.message}`);
        return false;
    }
}
/** Run Python certificate validator with SNI support */
async function runPythonCertificateValidator(host, port = 443) {
    try {
        ...
        log(`[tlsScan] Python validator failed for ${host}: ${error.message}`);
        return null;
    }
}
/** Parse sslscan XML output */
function parseSSLScanOutput(xmlOutput, host) {
    try {
        ...
        log(`[tlsScan] Failed to parse sslscan output: ${error.message}`);
        return null;
    }
}
/** Check if domain is behind CDN/proxy that terminates SSL */
async function isCloudFlareProtected(hostname) {
    try {
        ...
        try {
            ...
            for (const [service, indicators] of Object.entries(cdnIndicators)) {
                const matchesService = indicators.some(indicator => headerStr.includes(indicator) ||
        ...
        log(`[tlsScan] CDN detection failed for ${hostname}: ${error.message}`);
        return false;
    }
}
/** Get remediation advice for TLS issues */
function getTlsRecommendation(vulnerability) {
...
async function performCrossValidation(host, sslscanResult, pythonResult, scanId) {
    ...
    if (sslscanHasCert && !pythonHasCert) {
        ...
        await insertFinding({
            scan_id: scanId,
            type: 'TLS_VALIDATION_INCONSISTENCY',
            severity: 'MEDIUM',
            title: 'Certificate validation inconsistency detected',
            description: `sslscan: found cert, Python validator: ${pythonResult.error || 'validation failed'}`,
            data: {
                recommendation: 'Certificate found by sslscan but Python validation failed - investigate certificate validity',
                host: host,
                python_error: pythonResult.error
            }
        });
    ...
    if (!pythonResult.sni_supported && sslscanResult.certificate) {
        ...
        await insertFinding({
            scan_id: scanId,
            type: 'SNI_CONFIGURATION_ISSUE',
            severity: 'HIGH',
            title: 'SNI configuration issue detected',
            description: `Certificate found by sslscan but Python validator failed: ${pythonResult.error}`,
            data: {
                recommendation: 'Configure proper SNI support for cloud-hosted certificates',
                host: host,
                python_error: pythonResult.error
            }
        });
    ...
    if (pythonResult.certificate?.is_expired && sslscanResult.certificate && !sslscanResult.certificate.expired) {
        ...
        await insertFinding({
            scan_id: scanId,
            type: 'CERTIFICATE_EXPIRY_VERIFIED',
            severity: 'CRITICAL',
            title: 'Certificate expiry confirmed',
            description: `Python validator confirms certificate expired: ${pythonResult.certificate.not_after}`,
            data: {
                recommendation: 'Certificate expiry confirmed by Python validator - renew immediately',
                host: host,
                expiry_date: pythonResult.certificate.not_after
            }
        });
    }
    ...
    if (pythonResult.tls_version) {
        ...
        if (tlsVersion.includes('1.0') || tlsVersion.includes('1.1')) {
            ...
            await insertFinding({
                scan_id: scanId,
                type: 'WEAK_TLS_VERSION_NEGOTIATED',
                severity: 'HIGH',
                title: 'Weak TLS version detected',
                description: `Negotiated TLS version: ${tlsVersion}`,
                data: {
...
async function scanHost(host, scanId) {
    let findingsCount = 0;
    ...
    try {
        ...
        if (result.certificate) {
            ...
            if (cert.expired) {
                ...
                await insertFinding({
                    ...
                    description: `Certificate for ${host} expired on ${cert.notAfter}`,
                    data: {
                        recommendation: 'SSL certificate has expired - renew immediately',
                        host: host,
                ...
                if (severity) {
                    ...
                    await insertFinding({
                        ...
                        description: `Certificate for ${host} expires on ${cert.notAfter}`,
                        data: {
                            recommendation: `Certificate expires in ${daysUntilExpiry} days - plan renewal`,
                            host: host,
                            expiry_date: cert.notAfter,
            ...
            if (cert.selfSigned) {
                ...
                await insertFinding({
                    ...
                    description: `Self-signed certificate detected for ${host}`,
                    data: {
        ...
        for (const vulnerability of result.vulnerabilities) {
            ...
            if (vulnerability.includes('No SSL certificate')) {
                ...
                if (pythonData && pythonData.error?.includes('unable to get local issuer certificate')) {
                    ...
                    await insertFinding({
                        scan_id: scanId,
                        type: 'TLS_CONFIGURATION_ISSUE',
                        severity: 'MEDIUM',
                        title: 'TLS configuration issue detected',
                        description: `Python validation: ${pythonData.error}`,
                        data: {
                            recommendation: 'Configure server to present complete certificate chain including intermediate certificates',
                            host: host,
            ...
            await insertFinding({
                ...
                type: 'TLS_CONFIGURATION_ISSUE',
                severity: 'MEDIUM',
                title: 'TLS vulnerability detected',
                description: vulnerability,
                data: {
                    recommendation: getTlsRecommendation(vulnerability),
                    host: host,
        ...
        log(`[tlsScan] Scan failed for ${host}: ${error.message}`);
    }
...
export async function runTlsScan(job) {
    ...
    for (let i = 0; i < hostArray.length; i += MAX_CONCURRENT_TLS_SCANS) {
        ...
        const results = await Promise.allSettled(chunk.map(host => scanHost(host, job.scanId)));
        for (const result of results) {
            if (result.status === 'fulfilled') {
                totalFindings += result.value.findings;
                anyCert || (anyCert = result.value.hadCert);
            }
            else {
    ...
    if (!anyCert) {
        ...
        await insertFinding({
            scan_id: scanId,
            type: 'MISSING_TLS_CERTIFICATE',
            severity: 'HIGH',
            title: 'Missing TLS certificate',
            description: 'No valid SSL/TLS certificate found on any tested host variant',
            data: {
                recommendation: 'Configure SSL/TLS certificates for all public hosts',
                host: host
            }
        });
        totalFindings += 1;
